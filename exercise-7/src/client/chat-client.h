...
public:
  ...
  int send_message(const std::string &message);       // New helper
  std::string receive_message();                      // Optional helper
  void receive_thread();                             
  ~Client();

private:
  int socket_;
  std::atomic<bool> running{true};                    // This is Optional for clean exit, i just added
